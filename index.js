...
async function sendBotState(title, description, color = COLORS.info, extraFields = []) {
  try {
    // Always use the real logging function. This avoids the old
    // 'send massage is not defined' typo/reference completely.
    if (typeof sendLog !== 'function') {
      console.error('[BOT STATE] sendLog is not defined');
      return;
    }
